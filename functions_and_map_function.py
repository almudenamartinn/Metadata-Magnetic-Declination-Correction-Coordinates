...
def obtener_exif_datos(ruta_archivo):
    """
    Obtain the metadata from a photo.
    Args:
        ruta_archivo: File path of the photo.
    Returns:
        Specific metadata from the photo required for subsequent functions.
    """
...
def calcular_nueva_posicion(px, py, ancho_decimal, alto_decimal, latitud_decimal, longitud_decimal, h_decimal, ang_declin):
    ...
    Returns:
        Geographic coordinates after applying magnetic declination correction.
    """
...
def visualizacion_en_mapa(latitud_decimal, longitud_decimal):
    """
    Generate a map with a star at the exact coordinate.
    Args:
        latitud_decimal: Latitude of the central location of the image, in decimal format.
        longitud_decimal: Longitude of the central location of the image, in decimal format.
    Returns:
        A map visualization.
    """
    # High-resolution map (h) with Mercator projection
    map = Basemap(projection='merc', lat_0=latitud_decimal, lon_0=longitud_decimal,
                  resolution='h', area_thresh=0.1,
                  llcrnrlon=longitud_decimal - 5, llcrnrlat=latitud_decimal - 5,
                  urcrnrlon=longitud_decimal + 5, urcrnrlat=latitud_decimal + 5)
    ...
    x, y = map(longitud_decimal, latitud_decimal)
